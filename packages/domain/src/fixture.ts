...
export function createFixtureIR(): DesignIR {
  ...
  const node = (id: string, kind: 'stack' | 'grid' | 'cluster' | 'media' | 'type' | 'surface' | 'ornament' | 'component', semantic: string, props: Record<string, string | number | boolean>) => ({ id, kind, semantic, props, slots: {}, responsive: [] });
  return {
    ...
    pages: { routes: [
      { id: 'page-home', route: '/', title: 'Oficina — início', rootNodeId: 'home-root', nodes: [node('home-root', 'stack', 'main', { gap: '{space.lg}', color: '{color.ink}', background: '{color.paper}' }), node('home-title', 'type', 'h1', { text: 'Toda escolha tem motivo.', color: '{color.ink}', font: '{type.display}' }), node('home-proof', 'surface', 'section', { text: 'Processo rastreável.', background: '{color.accent}', radius: '{radius.card}', padding: '{space.md}' })] },
      { id: 'page-proof', route: '/proof', title: 'Oficina — prova', rootNodeId: 'proof-root', nodes: [node('proof-root', 'stack', 'main', { gap: '{space.md}', color: '{color.ink}', background: '{color.paper}' }), node('proof-title', 'type', 'h1', { text: 'Prova antes do brilho.', color: '{color.ink}', font: '{type.display}' })] },
      { id: 'page-contact', route: '/contact', title: 'Oficina — contato', rootNodeId: 'contact-root', nodes: [node('contact-root', 'stack', 'main', { gap: '{space.md}', color: '{color.ink}', background: '{color.paper}' }), node('contact-title', 'type', 'h1', { text: 'Vamos conversar.', color: '{color.ink}', font: '{type.display}' })] },
    ] },
