export const documentRules = {
  ...
  cssTokens: 'Every token path must compile to a distinct CSS custom property built from letters, digits and hyphens only, and no token value, once its aliases are resolved, may contain < > ; { } or a CSS comment delimiter, leave a string, a function or an escape open at its end, or close a construct it never opened.',
