...
export class RunPlanner {
  plan(runId: string, baseVersionId: string, brief: string): RunPlan {
    ...
    const tasks = stages.map((item, index) => ({ id: `task-${item.stage}`, ...item, state: 'queued' as const, lane: 'claude' as const, baseVersionId, inputDigest: hashJson({ runId, brief }), promptVersion: 'phase0-v1', modelAlias: 'claude-local', allowedPaths: ['/identity', '/pages', '/assets', '/reviewRecord'], brief }));
