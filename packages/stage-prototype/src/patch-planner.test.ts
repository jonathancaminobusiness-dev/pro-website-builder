...
describe('patch planner', () => {
  ...
  it('writes a crop only onto a raster asset that exists', () => {
    ...
    expect(plan([finding(crop)], ir).rejected[0]!.reason).toContain('Only a raster asset can be cropped');
    ir.assets.items[0] = { ...ir.assets.items[0]!, kind: 'raster' };
    ...
    expect(plan([finding({ ...crop, assetId: 'ghost' })], ir).rejected[0]!.reason).toContain('no asset ghost');
  });
  ...
  it('refuses a repair that leaves the allowlist, the token system or the identity vocabulary', () => {
    ...
    expect(reasons(plan([finding({ operation: 'set_token', nodeId: 'home-title', prop: 'gap', token: '{space.nope}' })]))).toContain('does not define the token');
    expect(reasons(plan([finding({ operation: 'set_token', nodeId: 'ghost', prop: 'gap', token: '{space.md}' })]))).toContain('no node ghost');
    expect(reasons(plan([finding({ operation: 'replace_copy', nodeId: 'home-title', text: 'Uma promessa revolucionário.' })]))).toContain('which the identity forbids');
    expect(reasons(plan([finding({ operation: 'set_constraint', nodeId: 'home-title', container: 'wide', rule: 'x' })]))).toContain('no container wide');
    expect(reasons(plan([finding({ operation: 'reorder_node', nodeId: 'home-root', slot: 'children', order: ['home-title', 'ghost'] })]))).toContain('must keep exactly the children');
    expect(reasons(plan([finding({ operation: 'set_token', nodeId: 'home-title', prop: 'gap', token: '{space.md}' })], createFixtureIR(), ['/reviewRecord']))).toContain('outside the paths this task may touch');
  });
  ...
  it('refuses a no-op, an abstention, a repairless finding and a low-confidence guess', () => {
    ...
    expect(reasons.map((entry) => `${entry.finding.id}: ${entry.reason}`).join(' | ')).toMatch(/already sets color/);
    expect(reasons.find((entry) => entry.finding.id === 'bare')!.reason).toContain('no minimal repair');
    expect(reasons.find((entry) => entry.finding.id === 'shy')!.reason).toContain('below the 0.5');
    expect(reasons.find((entry) => entry.finding.id === 'abstained')!.reason).toContain('escalated the decision to a human');
  });
  ...
  it('caps a cycle at three causal repairs and never lets two of them write the same path', () => {
    ...
    expect(collided.rejected.map((entry) => entry.reason)).toEqual(expect.arrayContaining([expect.stringContaining('already writes')]));
    ...
    expect(capped.rejected[0]!.reason).toContain('already carries 3 causal repairs');
  });
