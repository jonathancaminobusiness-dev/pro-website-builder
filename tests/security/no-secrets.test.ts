...
describe('phase 0 secret boundary', () => {
  it('keeps database dump, rendered bundle, and captured log data free of secret-like values', async () => {
    ...
    const run = new FixtureRun({ modelProvider: 'fake', repository: new ProjectRepository(database), release: releaseOptions(join(root, 'exports')), provider: createModelProvider() });
