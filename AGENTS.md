...
- There is one export path: `compileRelease` → `evaluateReleaseGate` → `writeReleaseBundle`. The finalization approval, Gate 3 and `run:release` all consult the gate's full veto set (compiler, evidence, divergence) before writing, so a new export must not write on `compiled.vetoes` alone or compile a sibling version.
