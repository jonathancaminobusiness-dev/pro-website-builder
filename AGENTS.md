...
- Every runner under `tests/release/` writes a typed artifact, pass or fail: the gate reads artifacts and nothing else, so a browser check that only fails a test is invisible to Gate 3.
