...
- There is one publish path: `ReleaseRun.publish`. Publishing the Gate 3 bundle *is* the finalization approval — `FixtureRun.approve` refuses that stage, and `publishRelease` claims the gate before its first await — so vetoes, the acceptance of open escalations, the `release.published` event and the single bundle root all live in one place. `run:release` is the only other writer and checks `report.blocked` first.
- Only `approverRole: 'captain'` may accept an open escalation. A script publishes as `'fixture'` and only when the report has no veto and no escalation, so no scripted run ever signs for the captain; `run:fixture` stops at Gate 3 and exits non-zero when anything is open.
