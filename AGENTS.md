...
- There is one publish path: `ReleaseRun.publish`. Publishing the Gate 3 bundle *is* the finalization approval — `FixtureRun.approve` refuses that stage — so vetoes, the captain's written acceptance of open escalations, the `release.published` event and the single bundle root all live in one place. `run:release` is the only other writer and checks `report.blocked` first.
