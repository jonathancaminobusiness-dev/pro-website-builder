...
`--json-schema` now carries a per-stage `AgentResult` schema: `operations` is a union in which each writable root path of that stage (`/identity` and `/reviewRecord` for identity, `/pages`, `/assets` and `/reviewRecord` for prototype, `/assets` and `/reviewRecord` for finalization) types `value` with the inlined subtree schema, and any other operation must address a path beneath one of them. `PatchGate` validates every proposal against the same per-stage Zod schema before the applier reads the document, so a wrong-shaped value is refused at the gate rather than by `designIRSchema` afterwards. The recursive DTCG token group is the one shape that cannot be inlined; `zod-to-json-schema` degrades it to `any` and says so on stderr.
...
The last 2026-09-07 run of that command, with the per-stage schema above, completed the whole journey. Claude produced a proposal for each of the three stages, all three passed the patch gate and the applier's dry run on the first attempt, and the captain gate approved each one: the event log holds `task.queued`, `task.started`, `patch.applied`, `version.created`, `task.succeeded` and `approval.recorded` for `identity`, `prototype` and `finalization`, with no `task.failed`, and ends at `run.finished` carrying the export digest. The database holds 3 patches, 3 tasks, 3 approvals and 4 versions (root plus one per stage). The command exited 0 and wrote `index.html`, `proof/index.html`, `contact/index.html` and `manifest.json` under `exports/<digest>/`. That run's identity stage replaced `/identity/meta/version`, the prototype stage replaced `/pages` as a whole subtree, and the finalization stage replaced `/assets/items/0/id`.
