...
Gate 1 starts with an editable Portuguese briefing, an example and a character counter. The Studio sends the trimmed value to `POST /api/identity/runs`; the server rejects non-text, blank or overlong values with a `4xx` response, and the supported limit is 8,000 characters. The briefing is stored with its execution, returned in every snapshot and restored after a server restart, so creating another run from a loaded execution reuses the current text. Older callers that omit `briefing` use the compatibility value `IDENTITY_BRIEFING`.
