...
A Gate 1 the captain leaves open outlives the browser tab and the server process. `IdentityRun.restore` rebuilds a run from the versions, approvals, events and one checkpoint the ledger already holds, the identity routes resolve a run through it, and creating over an id that is only on disk answers `409` instead of handing back an empty run. One run object exists per id: an id is reserved while its run is being created, and two cold requests that arrive together share the one rebuild rather than each deciding from a ledger the other has already moved on from. A run whose stage was still in flight when the process ended comes back `interrupted` and can be started again. The Gate 1 screen remembers the last run id and reopens it on load, and its empty state takes an existing id, so the decision does not need a hand-written request. Each direction card opens the isolated preview of its own version before anything is approved, so the comparison the alternative branches exist for is made on the three rendered documents and not on their token cards.
...
Three additive routes sit on the execution that already carries the free briefing:
...
The pipeline screen drives one fixture run. Its first button loads the fixed briefing and then reads `Novo briefing`, because it creates another run rather than restarting the one on screen. While a queued run's stage is in flight the screen offers `Cancelar execução` (`POST /api/runs/<id>/cancel`), and a cancelled run offers `Retomar execução` (`POST /api/runs/<id>/restart`) in place of the stage action, which resumes it from the same immutable revision — neither needs a hand-written request any more. The screen holds a snapshot rather than a subscription, so it re-reads `GET /api/runs/<id>` after each of those actions and whenever the tab returns to the foreground; a change made outside it therefore appears without a reload. It also remembers the execution this browser last worked on, the way Gate 1 remembers its own: it re-reads that run on load, so a reload finds the pipeline where it was left, and it forgets the pointer only when the server answers `404`. A restart rewinds the run to the last version the captain approved, so a proposal he had not yet decided is gone: the restored screen names the stage whose proposal was discarded and asks for it to be run again, instead of showing a rewound document as progress.
...
The Gate 2 screen is at `http://127.0.0.1:5173/#/gate-2`. It compares the composed revision with the refined one on the same route at the same width, offers an overlay and a difference blend, keeps the deterministic gate and the critics' opinion in separate panels, and records accept, reject or defer for each issue before the captain settles the gate. The reason is the captain's own: the Studio never writes one for him, so a decision and the gate itself are offered only once their reason is written, and `Aprovar o Gate 2` stays closed while any finding is undecided — the panel says how many are left. Both sides are always shown: when the loop applied no repair the two are the same revision and the difference blend is empty, which is itself the answer.
...
The review only offers what the run measured: `result.viewports` is the set of widths the evidence actually carried, so the A/B comparison cannot be opened at a width the deterministic gate never looked at. The comparison opens at one of those widths rather than a fixed 1440; a run that measured none says so instead of showing a comparison.
...
log and the release record keep. Only the captain publishes. Opening the panel
reads the report the run already holds, so a tab switch never costs the five
critics a second time; a run with no report yet simply has an empty panel.
