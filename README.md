...
The Gate 1 screen also accepts a written briefing for the identity run. It is trimmed, required, capped at 8,000 characters, stored with the run, and reused when a run is restored; older runs keep the legacy briefing automatically. The editor structure is composed by `packages/renderer` so the Studio supplies only its UI element factory while the renderer remains framework independent.
