...
Before the stage runs, the same screen holds a short briefing conversation ("Conversa de briefing"). The captain writes the initial text and the Studio answers with its reading — fact, hypothesis and unknown kept apart — then asks one question at a time, each saying why the answer changes the identity; the captain answers, corrects an earlier turn, skips the question or cancels the conversation with the history on screen. When the context is enough or a ceiling is reached, an editable consolidated summary appears and "Fechar briefing" closes it: that close is what enables "Executar etapa de identidade", and the confirmed summary becomes the text a new execution carries. The conceptual directions the conversation shows are text only — no preview opens before Gate 1 — and the message counter and the message/time ceilings are read from the snapshot the server sent rather than from any number in the interface. A server without the conversation endpoints answers `404`: the panel renders nothing and the free-text briefing above decides the run exactly as before.
