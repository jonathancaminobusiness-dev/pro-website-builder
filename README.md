...
The Gate 1 screen also accepts a written briefing for the identity run. The Studio requires a non-empty briefing; a supplied API briefing is trimmed and capped at 8,000 characters, stored with the run, and reused when a run is restored — restoring a run whose stored briefing only needed trimming also rewrites the canonical value back into the row, as a convenience that never keeps an otherwise readable run from being served. Callers that omit the field keep the legacy briefing automatically. Asking for a new run from a screen that already remembers one opens a confirmation with an empty briefing editor: a replacement is always written from scratch, so nothing the captain did not read and type is ever posted. An older run whose stored briefing is invalid — blank, whitespace-only or over the cap — is not migrated to any fallback: it restores as `unrecoverable`, shows an explicit invalid-briefing placeholder instead of the legacy briefing, explains that the data was preserved and that a new run is the way forward, and every state-changing route refuses it. The briefing editor and its creation/replacement controls are composed by `packages/renderer`, so the Studio supplies only its UI element factory while the renderer remains framework independent.

### Briefing conversation

Before the identity stage runs, an execution can carry a short, persisted conversation that turns the captain's free text into a briefing both sides understand. It is a preparation layer, not a fourth gate and not a preview generator: it answers in words only — positioning, tone, visual language, palette by function and mood, typography as intention, composition, applications — and `findVisualOutput` refuses any turn that tries to ship markup, a token path, a code fence or an embedded image, plus a hex value, a stylesheet declaration, an image file or a link the captain never wrote. Those four are the captain's to use: when a field that gives the briefing back — the reply, the question, the `summary`, the `facts`, the `hypotheses` or a declared gap — repeats a whole value the captain typed, it passes; a shortened or altered one — `#2E7` where the captain wrote `#2E7D32` — is the model's own and is refused. A rebrand that names its current hex value still closes its briefing and still gets its three conceptual directions, while the same value invented by the model is refused whole. Gate 1 keeps the visual work.

The contract is one module, `packages/domain/src/conversation.ts`, consumed by the server today and exported as `@pwb/domain/conversation` for the Studio slice to import, so the two cannot drift. It carries the seven states — `entry`, `recommendation`, `question`, `confirmation`, `final`, `cancelled`, `failed` — their legal transitions including the two controlled returns (`question -> recommendation` after an answer, `confirmation -> question` when the captain asks for an adjustment), the closed `BriefingConversationTurn` a model may answer with, the size limits, the request bodies and the endpoint paths.

Four additive routes sit on the execution that already carries the free briefing:

```
POST /api/identity/runs/<id>/conversation          # send a message, or answer / correct / skip / cancel
GET  /api/identity/runs/<id>/conversation          # resume the transcript without spending a turn
POST /api/identity/runs/<id>/conversation/confirm  # close the briefing
POST /api/identity/runs/<id>/conversation/reopen   # open the next conversation after a cancelled or failed one
```

Every POST here also requires `approverRole: "captain"`, like every other Gate 1 mutation: spending a model turn, archiving a round and signing the briefing the paid fan-out runs on are the captain's acts. The GET writes nothing and asks for nothing. Every POST requires an `idempotencyKey`: a 60-second model turn will be retried after a timeout, and the same key never buys a second turn — whether the first is still in flight or already finished, and across a restart, because the applied keys are persisted with the conversation. The opening turn may omit `message` entirely, in which case the briefing the execution was created with is the entry text.

The model sees only what the plan lists: the current message, the original and normalized briefing, the history still needed, the confirmed summary, the open gaps with their impact, the state and turn number, and a pt-BR facilitation instruction that separates fact from hypothesis from unknown. No credential, machine path, other execution or Gate 1 decision is a field in that prompt. Its answer is validated against the closed schema and against the rules the schema cannot state — where the conversation is and what it may do next — and a violation buys exactly one correction re-invocation. After that the conversation enters safe mode: it keeps the whole transcript, explains the failure in pt-BR and offers a summary built deterministically from what the captain actually said, which the captain can edit and close by hand. A second failure in a row ends the conversation as `failed`, which still closes into a briefing and never touches the execution or a confirmed revision.

Defaults chosen for this first cut: at most 6 model questions and 60 seconds per turn; reaching the question cap yields an editable summary rather than a silent approval, and a correction after it revises that summary instead of reopening a question; the captain may close with gaps still open, and the confirmation records them beside the briefing rather than rewriting the text the captain signed; confirming appends a new revision and never rewrites one already signed, and while the execution holds identity work made from its briefing — a fan-out in flight or the checkpoint one left behind — the briefing is frozen: every conversation route answers 409 in pt-BR before any turn is bought, and the freeze is asked again inside the critical section the confirmation writes and applies in — the same section a start takes to claim the stage — so a start can never land between that answer and the briefing being applied, and a confirmation that raced a start never replaces a running stage. A stage that failed or was interrupted froze nothing: the captain confirms the next revision on the same execution and the next start runs on it. A stopped execution refuses a turn like every other route; and the three conceptual directions arrive with the `final` state on the same conversation. The whole conversation is written to the `conversation` column of `runs` after every change, so `IdentityRun.restore` rebuilds its text, transcript, state, summary, answered questions and confirmations from the execution. A confirmation writes that record, the applied idempotency key and the execution's `briefing` column in one transaction, and the execution picks the briefing up only after it commits: no run ever carries a briefing its transcript does not name, and a failed write leaves the same key free to re-drive the whole confirmation.

Cancelling and failing are terminal for the conversation they end and for nothing else: the execution is never deleted and stays reopenable. A cancel on a conversation the captain never wrote in closes nothing at all — it leaves the chat unopened, so the execution keeps the legacy flow and still starts on the briefing it was created with. `POST .../conversation/reopen` opens the next round on the same execution — `canReopenBriefingConversation` is the shared predicate for the two states it accepts — and it buys no model turn and signs nothing: the closed round is archived whole into `previousRevisions`, so everything said in it stays readable, `revision` moves to the next number, and the new round starts at `entry` exactly as the first one did. What the execution already carries is untouched: the revisions it has signed, the briefing it runs on and the idempotency keys it has spent. A briefing already signed is not reopened but revised, by confirming the next revision, so `final` is refused there.

A conversation row the server cannot read back — not JSON, or JSON this contract refuses — is neither dropped nor guessed at: the execution is marked unreadable, the snapshot carries `unreadable.reason`, every route that would write over it answers 409 in pt-BR and `IdentityRun.start` refuses, because a record nobody can read may be the cancelled round whose transcript is still on disk. `POST .../conversation/reopen` is the one route it still answers, and it destroys nothing: the bytes that could not be read are archived verbatim as a `previousRevisions` entry closed as `unreadable`, with the reason they were refused, and the captain says again in the next round what that record no longer shows them. That entry is numbered like every other: a round's `revision` is its position in `previousRevisions`, counted from 1 in the order the rounds were archived, and the current round is that count plus one — one rule, so the numbers are monotonic and none is ever invented. The number the damaged record claimed for itself is kept beside the bytes as `unreadable.claimedRevision`, informational and absent when it claimed nothing; it never decides a position.

The hand-off to Gate 1 is the confirmation and nothing else. Once an execution's captain has used the chat at all, `IdentityRun.start` runs only on a conversation that reached `final`: an open round is a draft and not a signature — the question cap offers an editable summary, which starts nothing — a cancelled round advanced nothing to the curator, and a failed one signed nothing either, and each refusal names the way forward that execution still has. An execution whose captain never opened the chat is the legacy flow and is untouched: a caller that omits the field keeps the fixed compatibility briefing and starts the stage on it exactly as before. What the curator then receives is the confirmed record itself, byte for byte — `restore` reads the briefing back from the transcript, so a restarted process runs the stage on the text the captain signed — together with the execution id it is curating and the instruction not to invent an audience, a proof or a restriction: anything the briefing does not state belongs in `unknowns` or in a risk-rated assumption and never in evidence. The chat writes no HTML, JSX, document, token or patch at any point; Gate 1 keeps the visual work and the preview.
