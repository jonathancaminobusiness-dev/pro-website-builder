...
`zod-to-json-schema` cannot express a `superRefine`, so the structural rules in `documentRules` — the media/figure pairing, the phrasing leaf rule, page-graph reachability and node id uniqueness, and page id/route uniqueness — reach the worker as prompt text built from that same object the gate quotes in its rejection messages. Three runs of the command on 2026-09-07 after that line was added were aborted by the per-stage deadline rather than reaching a gate: the first after the identity stage was approved and while the prototype stage was running, the other two during the identity stage, each leaving `task.started` as the last event and no patch committed. The cause is local, not the contract: `claude -p 'Reply with the single word: ok' --output-format json --max-turns 1` reported `duration_ms: 3390` for the API call and took 1m47s of wall clock, so roughly 100s of per-invocation process overhead was consuming the 5-minute stage budget. The prompt text above has therefore not yet been exercised end to end against the binary.
