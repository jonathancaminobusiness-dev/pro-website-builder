...
Defaults chosen for this first cut: at most 6 model questions and 60 seconds per turn; reaching the question cap yields an editable summary rather than a silent approval; the captain may close with gaps still open, and the briefing then lists them under `Lacunas declaradas em aberto:`; confirming appends a new revision and never rewrites one already signed; and the three conceptual directions arrive with the `final` state on the same conversation. The whole conversation is written to the `conversation` column of `runs` after every change, so `IdentityRun.restore` rebuilds its text, transcript, state, summary, answered questions and confirmations from the execution.
