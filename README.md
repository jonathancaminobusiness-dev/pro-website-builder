...
The command writes a local SQLite database under `.treehouse/` and a content-addressed export under `exports/`. Set `PWB_DB_PATH` and `PWB_EXPORT_ROOT` to use explicit locations.
...
CI and the fixture use `FakeModelProvider`. To exercise the real adapter, install and log in to the unmodified Claude Code binary as its owner, verify `claude --version`, then configure the server/provider entry point for a local `ClaudeRunner`. The runner uses `execFile` with no shell, a fresh session UUID, `--no-session-persistence`, structured JSON, schema validation, deadlines, and abort signals. It never reads, stores, prints, forwards, or asks for tokens or credentials. No paid API is required by this repository.
