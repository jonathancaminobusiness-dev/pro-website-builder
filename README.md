...
Generation is off until the owner names an MCP server, so a fresh checkout and the test suite never reach the network. `McpToolTransport` in `packages/providers` is the client: it opens a session, calls one tool and closes again, over JSON-RPC on either transport the MCP specification defines. Three environment variables reach it, checked in this order by `createRasterProvider`:

```bash
PWB_HIGGSFIELD_MCP_COMMAND=/path/to/server   # stdio; PWB_HIGGSFIELD_MCP_ARGS holds space-separated arguments
PWB_HIGGSFIELD_MCP_URL=https://example/mcp   # streamable HTTP
PWB_HIGGSFIELD_MCP=claude-code               # reuse the `higgsfield` server the owner's Claude Code declares
```

The third reads `~/.claude.json` for that server's endpoint alone — its `url`, or its `command` and `args` — and nothing else in the file. The MCP server owns its own authentication: this process composes no authorization header, reads no token, keeps the server's stderr unread rather than logging it, and reports a refused request by status alone because an endpoint may carry a credential in its query. A server that refuses, errors or cannot start makes that one image a `failed` asset with the reason on its provenance; the captain's decision is still recorded, and the next approval asks for the image again under the same digest. On this machine the owner's Claude Code declares `higgsfield` as an HTTP MCP server at `https://mcp.higgsfield.ai/mcp` and `claude mcp list` reports it as needing authentication, so no generation has been observed against the live server yet; every path above is covered against a fake MCP server over both transports.
