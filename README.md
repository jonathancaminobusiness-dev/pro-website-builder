...
**Gate 2 runs on what Gate 1 approved.** `POST /api/prototype/runs` names the identity execution (`identityRunId`) or the version that gate closed on (`versionId`), and the run is seeded from that approved document, whose own version id becomes its root — so every revision it produces descends from what the captain decided. A request that names neither, names a Gate 1 the captain never decided, names one whose identity moved after the decision, or names one whose approved imagery is still on the raster lane is answered `409`, never with the built-in fixture. The seeded document carries the imagery that lane delivered; an image it could not deliver stays out of the document — an asset with no bytes and no provider licence would reach the public bundle as a licence warning — and the run reports it on `chain.failedImagery`. The review reports the identity hash *measured* from the revision under review, so the screen states that the identity Gate 1 recorded is the one that was measured rather than assuming it. Approving the gate writes an `approvals` row under the chain's own run id — the identity execution's — together with the lineage between the two versions, which is what opens Gate 3. A review is decided exactly once: a second decision on a settled run is answered `409`, so a screen still holding the pre-decision snapshot cannot overwrite the row the release gate reads, and a revision the captain wants to revisit is measured in a new run. Gate 1 answers the same `409` to a direction returned after it closed, for the same reason.
