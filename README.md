...
Defaults chosen for this first cut: at most 6 model questions and 60 seconds per turn; reaching the question cap yields an editable summary rather than a silent approval, and a correction after it revises that summary instead of reopening a question; the captain may close with gaps still open, and the confirmation records them beside the briefing rather than rewriting the text the captain signed; confirming appends a new revision and never rewrites one already signed, and once the identity stage of that execution has started the briefing is frozen — the confirm route answers 409 in pt-BR and the captain works on a new execution; and the three conceptual directions arrive with the `final` state on the same conversation. The whole conversation is written to the `conversation` column of `runs` after every change, so `IdentityRun.restore` rebuilds its text, transcript, state, summary, answered questions and confirmations from the execution.
