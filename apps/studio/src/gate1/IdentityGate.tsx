  ...
  return <section className="gate-panel" aria-labelledby="gate1-title">
    ...
    {snapshot && <>
      ...
      <div className="actions gate-actions">
        ...
        <button className="primary" onClick={props.onStart} disabled={props.busy || props.inFlight || props.startRecoveryPending || running || stopped || unrecoverable || briefingOpen || snapshot.directions.length > 0}>
          {unrecoverable ? 'Execução encerrada' : stopped ? 'Execução cancelada' : snapshot.directions.length > 0 ? 'Etapa executada' : running ? 'Etapa em execução' : props.startRecoveryPending ? 'Verificando execução…' : briefingOpen ? 'Feche o briefing para executar' : failed ? 'Tentar novamente' : props.inFlight ? 'Iniciando…' : props.busy ? 'Executando…' : 'Executar etapa de identidade'}
        </button>
      </div>
