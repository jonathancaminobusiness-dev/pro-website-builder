...
export default function App() {
  ...
  async function act(action: () => Promise<Snapshot>): Promise<void> {
    ...
    try { commitSnapshot(await action()); } catch (cause) { setError(failureMessage(cause)); } finally { setBusy(false); }
  }
  ...
  // curl, and a stopped run is resumed from the same immutable revision — the
  // only next step a cancelled run has, so it takes the place of the stage
  // action the server would refuse. Both answer with a snapshot, and both are
  // followed by a read so the screen shows the state the server settled on
