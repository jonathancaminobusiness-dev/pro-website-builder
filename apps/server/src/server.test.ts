...
describe('local API', () => {
  ...
  it('serves a persisted run after a restart instead of answering 404', async () => {
    ...
    const server = createApiServer({
      ...
      loadRun: async (id) => { const cached = runs.get(id); if (cached) return cached; const run = new FixtureRun({ modelProvider: 'fake', repository, release: releaseOptions(join(dir, 'exports')), provider: new FakeModelProvider() }); if (!await run.restore(id)) return undefined; runs.set(id, run); return run; },
    });
    await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
    const address = server.address();
    const origin = `http://127.0.0.1:${typeof address === 'object' && address ? address.port : 0}`;
