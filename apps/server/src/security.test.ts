...
import { previewHeaders, STUDIO_ORIGIN } from './security.js';

describe('preview security boundary', () => {
  it('keeps a strict CSP that still lets the studio origin frame the preview', () => {
    ...
    expect(frameAncestors).toBe(`frame-ancestors ${STUDIO_ORIGIN}`);
