...
export class ProjectRepository {
  ...
  async updateRunBriefing(runId: string, briefing: string): Promise<void> { await this.write(() => { this.db.sqlite.prepare('UPDATE runs SET briefing = ? WHERE id = ?').run(normalizeIdentityBriefing(briefing), runId); }); }
