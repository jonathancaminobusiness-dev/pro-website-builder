...
export class ProjectRepository {
  ...
  async createRun(input: RunInput): Promise<void> { await this.write(() => { this.db.orm.insert(schema.runs).values({ id: input.id, projectId: input.projectId, briefing: normalizeIdentityBriefing(input.briefing, input.briefing !== undefined), createdAt: new Date().toISOString() }).run(); }); }
  async updateRunBriefing(runId: string, briefing: string): Promise<void> { await this.write(() => { this.db.sqlite.prepare('UPDATE runs SET briefing = ? WHERE id = ?').run(normalizeIdentityBriefing(briefing, true), runId); }); }
