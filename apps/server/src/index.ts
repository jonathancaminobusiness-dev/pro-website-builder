...
export async function startServer(options: { dbPath?: string; exportRoot?: string; apiPort?: number; previewPort?: number } = {}): Promise<{ api: ReturnType<typeof createApiServer>; preview: ReturnType<typeof createPreviewServer>; close: () => Promise<void> }> {
  ...
  const api = createApiServer({ runs, createRun: async (id) => { const run = new FixtureRun({ repository, exportRoot }); await run.initialize(id); runs.set(id, run); return run; } });
